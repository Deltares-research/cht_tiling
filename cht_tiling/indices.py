...
def make_index_tiles_quadtree(grid, path, zoom_range, format):
    ...
    for ilev in range(0, nr_refinement_levels):
        ...
        ifirst[ilev] = np.where(grid["level"].values[:] == ilev + 1)[0][0]
    ...
    for level in range(nr_refinement_levels):
        ...
        i0_lev.append(i0)    
        i1_lev.append(i1)    
        nmax_lev.append(np.amax(grid["n"].values[i0:i1]) + 1)
        mmax_lev.append(np.amax(grid["m"].values[i0:i1]) + 1)
        nn = grid["n"].values[i0:i1] - 1
        mm = grid["m"].values[i0:i1] - 1
