...
def webmercator_to_lat_lon(easting, northing):
    ...
    lat = (180 / math.pi) * (2 * math.atan(math.exp(northing / 20037508.34 * math.pi)) - (math.pi / 2))
    return lat, lon
...
def xy2num(easting, northing, zoom):
    ...
    ix, iy = lat_lon_to_tile_indices(lat, lon, zoom)
    return ix, iy
...
def elevation2png(val,
    ...
    if encoder == "terrarium":
        ...
        if np.any(val >= 255):
            # Throw an error
            raise ValueError("Some values in are equal to or larger than 255. This is not allowed for encoder 'uint8'.")
        ...
        if np.any(val >= 65535):
            # Throw an error
            raise ValueError("Some values are equal to or larger than 65535. This is not allowed for encoder 'uint16'.")
        ...
        if np.any(val >= 16777215):
            # Throw an error
            raise ValueError("Some values are equal to or larger than 16777215. This is not allowed for encoder 'uint24'.")
        ...
        if np.any(val >= 4294967295):
            # Throw an error
            raise ValueError("Some values are equal to or larger than 4294967295. This is not allowed for encoder 'uint32'.")
...
def png2elevation(png_file,
    ...
    if encoder == "terrarium":
        ...
        elevation = (rgb[:, :, 0] * 16777216 + rgb[:, :, 1] * 65536 + rgb[:, :, 2] * 256 + rgb[:, :, 3])
        ...
        i = rgb[:, :, 0] * 16777216 + rgb[:, :, 1] * 65536 + rgb[:, :, 2] * 256 + rgb[:, :, 3]
